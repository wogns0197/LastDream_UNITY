...
- 201107
  ...
  2) 모든 해상도 변경완료
  3) 점프버튼 수정
  4) 유니티 빌드를 iOS로 설정. --> 윈도우에서 실행 시 다르게 실행될 수 있습니다  
  
- 201110
  1) 일시정지 버튼 시 조작버튼 사라지게
  2) 점프 시 허들 위치 표시해주는 세모 표시 ( 엄청난 실력 향상 ) : 자식 오브젝트를 부모 오브젝트의 국한되지 않게 다른 오브젝트의 value에 의존성 부여 --> 나중에 써먹기
  3) 무적 발동 시 (특히 떨어졌을 때) 다시 떨어지지 않게 깜빡이 벽 설치
...
>>>>>>> 2e094f1b51066e6c2547d17354cb4b168d391aa6
